...
def create_conv_net(x, keep_prob, channels_in,channels_out, n_class, layers=2, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    # Output Map
    weight = weight_variable([1, 1, features_root, n_class*channels_out], stddev)
...
class Unet(object):
    ...
    def __init__(self, channels_in =5, channels_out=4,n_class=256,**kwargs):
        ...
        logits=tf.reshape(logits,[dims[0],dims[1],dims[2],channels_out,self.n_class],'reshape_before_softmax')
        self.predicter=tf.nn.softmax(logits,dim=-1,name='Softmax_multi_label')
        self.predicter=tf.argmax(self.predicter,4)
        self.correct_pred=tf.equal(tf.argmax(tf.nn.softmax(logits,dim=-1),4), tf.cast(self.y,tf.int64))
        self.accuracy=  tf.reduce_mean(tf.cast(self.correct_pred, tf.float32))  

        self.cost=tf.reduce_mean(cross_entropy(tf.reshape(tf.one_hot(tf.cast(self.y,tf.int64),n_class,on_value=1.0,off_value=0.0,axis=-1), [-1, n_class]),tf.reshape(tf.nn.softmax(logits,dim=-1), [-1, n_class])))
        self.gradients_node = tf.gradients(self.cost, self.variables)

        self.cross_entropy=self.cost       
    ...
    def predict(self, model_path, x_test):
        ...
        :returns prediction: The unet prediction Shape [n, px, py, channels_out] (px=nx-self.offset/2) 
        """
...
class Trainer(object):
    ...
    def __init__(self, net, batch_size=10, optimizer="momentum", opt_kwargs={}):
        self.net = net
    ...
    def train(self, data_provider, output_path, training_iters=40, epochs=100, dropout=0.75, display_step=1, restore=False, write_graph=False):
        """
        ...
        with tf.Session() as sess:
            ...
            test_x, test_y = data_provider(self.verification_batch_size) #verify the minibatch works
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch*training_iters), ((epoch+1)*training_iters)):
                    ...
                    if step % display_step == 0:
                        self.output_minibatch_stats(sess, summary_writer, step, batch_x, util.crop_to_shape(batch_y, pred_shape),epoch)
    ...
    def store_prediction(self, sess, batch_x, batch_y, name):
        ...
        for idx in range(batch_y.shape[0]):
            ...
            util.save_image(img_true, "%s/%s_true_%s0.jpg"%(self.prediction_path, name,idx))
            util.save_image(img_pred, "%s/%s_pred_%s0.jpg"%(self.prediction_path, name,idx))        
        return pred_shape
    
    def output_epoch_stats(self, epoch, total_loss, training_iters, lr):
        logging.info("Epoch {:}, Average loss: {:.4f}, learning rate: {:.4f}".format(epoch, (total_loss / training_iters), lr))
    
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y,epoch):
...
def error_rate(predictions, labels):
    ...
    return 100.0 - (
        100.0 *
        np.sum(predictions == np.asarray(labels,dtype='uint64')) /
